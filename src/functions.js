...
export function initializeEnvironment(cam, controls, planetsArray, ray, mouseVec, sunHalo, haloBloom, render, clk, p_sizes, p_camera, followedPlanet, camera_vr) {
    camera = cam;
...
function handleMovement(delta, camera_vr, camera) {
    ...
    for (const source of session.inputSources) {
        ...
        const joyX = axes[2] || axes[0];
        const joyY = axes[3] || axes[1];
        ...
        if (source.handedness === 'left') {
            ...
            camera_vr.position.addScaledVector(forwardFlat, -joyY * moveSpeed * delta);
...
export function animate(scene, camera, planetList, sun, stars, galaxy, camera_vr) {

    renderer.setAnimationLoop(() => {
        const delta = clock.getDelta();

        if (renderer.xr.isPresenting) {
            handleMovement(delta, camera_vr, camera); 
        } else {
        ...
        if (followedPlanet && !renderer.xr.isPresenting) {
